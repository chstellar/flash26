...
write_blot_plotter <- function(plotter, output_path) {
  ...
  hook <- c(
    ...
    "  }",
    "  selection_keys <- blot_selection %>%",
    "    rename(selected_cluster = cluster)",
    "  keep <- tbl %>%",
    "    mutate(metadata_category = as.character(metadata_category),",
    "           feature = as.character(feature)) %>%",
    "    inner_join(selection_keys, by = c('metadata_category', 'feature'), relationship = 'many-to-many')",
    "  if ('cluster' %in% colnames(keep)) {",
    "    keep <- keep %>% mutate(cluster = selected_cluster) %>% select(-selected_cluster)",
    "  } else {",
    "    keep <- keep %>% rename(cluster = selected_cluster)",
    "  }",
