...
known_causes = "penicillin|pbp"

# e faecium ampicillin result
setwd("/scratch/users/dcotter1/FLASH-runs/FLASH-grouped-changeReordering-moreClusters/metaSPLASH_pipeline/")
opt$nonzero_annotations = "results/eFaecium-CollEtAl-binary/filter1/shiftDist-levFilter/hyena/normalized/eFaecium-CollEtAl-binary_hyena_adelie_results_top50000_target1_k54_s54_trainProp0.8_nonzero_coefficients_blastp_annotated.tsv"
opt$clusters = "results/eFaecium-CollEtAl-binary/filter1/shiftDist-levFilter/eFaecium-CollEtAl-binary_sequences_per_cluster_top50000-clusters_target1_k54_s54.tsv"
opt$feather = "/scratch/users/dcotter1/test_FLASH-smk_feb2026/rerun-change-reordering/eFaecium-CollEtAl-binary/eFaecium-CollEtAl-binary_hyena_top_variance_features_for_glmnet_filter1_shiftDist-levFilter_top50000_target1_k54_s54_normalized.feather"
opt$sample_seqs = "/scratch/users/dcotter1/test_FLASH-smk_feb2026/rerun-change-reordering/eFaecium-CollEtAl-binary/eFaecium-CollEtAl-binary_prepared_sequences_filter1_shiftDist-levFilter_top50000_target1_k54_s54_sample_sequences.tsv"
opt$metadata = "/oak/stanford/groups/horence/dcotter1/utility_files/metadata/metaSPLASH_metadata/E_faecium_cleaned_resistance_metadata_binary.tsv"
opt$output = "/oak/stanford/groups/horence/dcotter1/share/250313/test_eFac_blast_out.pdf"
category = "vancomycin_RIS"

# s pneumo ampicillin result 
setwd("/scratch/users/dcotter1/FLASH-runs/FLASH-grouped-changeReordering-moreClusters/metaSPLASH_pipeline/")
opt$nonzero_annotations = "results/pneumo-ERP001505/filter1/shiftDist-levFilter/hyena/normalized/pneumo-ERP001505_hyena_adelie_results_top50000_target1_k54_s54_trainProp0.8_nonzero_coefficients_blastp_annotated.tsv"
opt$clusters = "results/pneumo-ERP001505/filter1/shiftDist-levFilter/pneumo-ERP001505_sequences_per_cluster_top50000-clusters_target1_k54_s54.tsv"
opt$feather = "/scratch/users/dcotter1/test_FLASH-smk_feb2026/rerun-change-reordering/pneumo-ERP001505/pneumo-ERP001505_hyena_top_variance_features_for_glmnet_filter1_shiftDist-levFilter_top50000_target1_k54_s54_normalized.feather"
opt$sample_seqs = "/scratch/users/dcotter1/test_FLASH-smk_feb2026/rerun-change-reordering/pneumo-ERP001505/pneumo-ERP001505_prepared_sequences_filter1_shiftDist-levFilter_top50000_target1_k54_s54_sample_sequences.tsv"
opt$metadata = "/oak/stanford/groups/horence/dcotter1/utility_files/metadata/metaSPLASH_metadata/pneumonia_augmented_metadata.tsv"
opt$output = "/oak/stanford/groups/horence/dcotter1/share/250313/test_eFac_blast_out.pdf"
category = "Phenotypic_Penicillin"

# H5n1 result 
setwd("/scratch/users/dcotter1/FLASH-runs/FLASH-grouped-changeReordering-moreClusters_H5N1/metaSPLASH_pipeline/")
opt$nonzero_annotations = "results/H5N1-cattle/filter1/shiftDist-levFilter/hyena/normalized/H5N1-cattle_hyena_adelie_results_top50000_target1_k54_s54_trainProp0.8_nonzero_coefficients_blastp_annotated.tsv"
opt$clusters = "results/H5N1-cattle/filter1/shiftDist-levFilter/H5N1-cattle_sequences_per_cluster_top50000-clusters_target1_k54_s54.tsv"
opt$feather = "/scratch/users/dcotter1/test_FLASH-smk_feb2026/rerun-change-reordering/H5N1-cattle/H5N1-cattle_hyena_top_variance_features_for_glmnet_filter1_shiftDist-levFilter_top50000_target1_k54_s54_normalized.feather"
opt$sample_seqs = "/scratch/users/dcotter1/test_FLASH-smk_feb2026/rerun-change-reordering/H5N1-cattle/H5N1-cattle_prepared_sequences_filter1_shiftDist-levFilter_top50000_target1_k54_s54_sample_sequences.tsv"
opt$metadata = "/oak/stanford/groups/horence/dcotter1/utility_files/metadata/metaSPLASH_metadata/H5N1_metadata_from_Nguyen_et_al_2024_PRJNA1102327_modified250528.tsv"
#opt$output = "/oak/stanford/groups/horence/dcotter1/share/250313/test_eFac_blast_out.pdf"
category = "cleaned_host"
...
for (category in categories) {
  tryCatch({
    ...
    plot_dt <- summ_dt %>%
      ...
      mutate(label = str_wrap(str_trunc(label, width =50, side="right"), width = 30)) %>% 
    ...
    if (FALSE) {
      custom_labels = c("hypothetical protein",
                        "rpmA" ,
                        "PBP family protein",
     ...
     custom_labels <- c(
       "VanS-type",
       "VanA-type vancomycin\nresistance histidine kinase",
       "D-alanine--(R)-lactate ligase\nVanB",
       "topoisomerase C-terminal\nrepeat-containing protein",
     ...
     custom_labels <- c(
      ...
      plot_dt <- plot_dt %>% mutate(label = ifelse(rank <=10 , custom_labels, label))
    ...
    p <- plot_dt %>% head(opt$num_hits) %>%
      ...
      geom_col() + 
      geom_text(aes(y=coef_mag + 0.05,hjust=0),angle=65,size=3.5,color="grey30",fontface="bold") +
      scale_y_continuous("Magnitude relative to\nlargest nonzero coefficient", 
                         limits = c(0,2.25), 
                         labels=scales::label_percent(), breaks=seq(0,1,0.25),
                         expand=c(0,0)) +
      xlab("Cluster Number") +
      scale_fill_manual(breaks=c("known_cause","blast",NA), values=c("darkred", "pink", "grey"),
                        labels=c("Known Cause", "Blast hit", NA)) +
      scale_x_continuous(breaks=seq(1,10,1), expand=c(0.05,0.01)) +
      coord_cartesian(xlim=c(1,12.2)) +
      ggtitle(make_title,
      ...
      theme(legend.position="none",
            ...
            legend.text = element_text(color="grey30",face="bold",size=14), 
            legend.title = element_text(face="bold",size=14),)
    
    print(p)
    
     filename=paste0("/oak/stanford/groups/horence/dcotter1/FLASH_paper_figures/blast_scatter_plots/H5N1_shiftDist_cleaned_host_top10_clusters_barchart.pdf")
     ggsave(p, filename= filename, width=8,height=5.5,units="in")
    ...
    for (j in 1:nrow(interesting_clusters)) {
      ...
      if (length(unique(p_sub$`BLAST Label`)) <= 6) {
        ...
        p2 <- p_sub %>% 
          mutate(`BLAST Label` = ifelse(str_detect(`BLAST Label`, "penicillin"), "PBP5", `BLAST Label`)) %>%
          ggplot(aes(x=embedding, y=lev_dist, color=prop_first_class,# shape=`BLAST Label`,
                    size=total_samples,
          ...
          theme_minimal() + xlab(expression(bold("Embedding" ~ "\u00D7" ~ beta))) +
          ylab("Levenshtein Distance\n(to most abundant anchor-target)") + #ggtitle(my_cluster) +
          theme(axis.text = element_text(face="bold",size=12), 
                axis.title = element_text(face="bold",size=12),
                legend.text = element_text(color="grey30",face="bold",size=12), 
      ...
      print(p2)
      filepath = "/oak/stanford/groups/horence/dcotter1/FLASH_paper_figures/blast_scatter_plots/H5N1_cluster7_HA1_scatter_plot.pdf"
      ggsave(p2, filename=filepath, height=5, width=8)
