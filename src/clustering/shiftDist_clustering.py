        ...
        for i in table_1[anch]:
            if anch in table_2:
                for j in table_2[anch]:
                    ...
                    cols.append(j)
    data = [True] * len(rows)
    update_adj_table = scipy.sparse.csr_matrix((data, (rows, cols)), shape=(n,n), dtype=bool)
...
def cluster_anchors(anchor_list, max_shift_dist=5):
    ...
    assert np.asarray([len(anchor) == k for anchor in anchor_list]).all()  # assert all sequences are same length
    ...
    logger.info(f'Time taken for networkx graph construction: {networkx_time - adj_mat_time}')
...
def _sparse_size(matrix):
    return sys.getsizeof(matrix.data) + sys.getsizeof(matrix.indices) + sys.getsizeof(matrix.indptr)
