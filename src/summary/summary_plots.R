...
data1 <- fread(cmd=load_cmd, header=FALSE, sep="\t", 
              col.names = c("path_metadata", "feature", "accuracy",
                            "sensitivity", "specificity", "classes", "coefficients"))
...
pdf("results/summary_plots/eFaecium_sensitivity_comparison_241119.pdf")
for (i in all_cols) {
  y_col = sym(i)
  p <- plot_data %>% 
    ggplot(aes(x=`ohe (50000 clusters)`,y=!!y_col)) + geom_abline(slope=1, intercept = 0, color="grey", lty="dashed") + 
