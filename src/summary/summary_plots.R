...
data %>% distinct(dataset, metadata) %>% arrange(dataset) -> unique_sets
...
for (i in 1:nrow(unique_sets)) {
  ...
  my_dataset=unique_sets[i,]$dataset
  my_metadata = unique_sets[i,]$metadata
  if (my_dataset != curr_dataset) {
    k=0
  }
  if (k==0) {
    check =
    plot(0:10, type = "n", xaxt="n", yaxt="n", bty="n", xlab = "", ylab = "")
    text(5, 10, paste(my_dataset), cex=1.5, font=2)
    curr_dataset=my_dataset
    k=1
  }
  p1 <- temp_data %>% ggplot(aes(x=paramater_set, y=accuracy, shape=filter, color=model)) +
    geom_point(size=3) + theme_minimal() + theme(axis.text.x = element_text(angle = 90, hjust = 1, size=6)) + coord_cartesian(ylim=c(0,1)) + 
    ggtitle(my_metadata)
...
# compare beta coefficients across models
## plot all data in one plot
...
get_max_coef <- function(x) {
  betas = strsplit(gsub("\\[|\\]", "", x),split = ",")[[1]]
  max_beta = betas[which.max(abs(as.numeric(betas)))]
  return(max_beta)
}

data <- rbind(data1,data2)
data <- data %>% rowwise() %>% mutate(max_beta = get_max_coef(coefficients)) %>% 
  separate(path_metadata, into=c("path", "metadata"), sep=":") 
...
data %>% filter(model!="ohe") %>% 
  select(-c(accuracy, sensitivity, specificity, classes, coefficients)) %>% 
  group_by(dataset,metadata,num_clusters,filter,cluster_approach) %>% 
  pivot_wider(values_from = max_beta, names_from=model) %>% View()
