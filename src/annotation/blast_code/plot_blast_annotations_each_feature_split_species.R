...
option_list <- list(
  ...
  make_option(c("--split_metadata_col"), type="character", default="species",
              help = "Metadata column used to split detailed embedding-by-distance plots into separate pages.", metavar = "character")
)
...
split_metadata_col_override <- Sys.getenv("FLASH_SPLIT_METADATA_COL", unset="")
if (split_metadata_col_override != "") {
  opt$split_metadata_col <- split_metadata_col_override
}
...
split_metadata_col <- opt$split_metadata_col
if (!is.null(split_metadata_col) && split_metadata_col != "" && !(split_metadata_col %in% names(all_metadata))) {
  stop(paste0("Split metadata column '", split_metadata_col, "' was not found in ", opt$metadata))
}

cols_to_keep <- which(names(all_metadata) %in% c("sample_name", categories, split_metadata_col))
...
for (category in categories) {
  tryCatch({
    ...
    # filter metadata for only current category plus the custom split column
    my_metadata <- as_tibble(all_metadata) %>%
      select(sample_name, !!category, !!sym(split_metadata_col)) %>%
      dplyr::rename(metadata:=!!category, split_metadata:=!!sym(split_metadata_col))
    for (j in 1:nrow(interesting_clusters)) {
      ...
      summ_sub_dt <- seq_sub %>% left_join(my_metadata,by="sample_name") %>% 
        filter(!is.na(split_metadata) & split_metadata != "") %>%
      ...
      for (split_value in sort(unique(p_sub$split_metadata))) {
        p_sub_split <- p_sub %>% filter(split_metadata == split_value)
        plot_title <- paste(my_cluster, split_metadata_col, split_value, sep=" | ")

        if (length(unique(p_sub_split$`Blast Label`)) <= 6) {
          p2 <- p_sub_split %>%
      ...
      summ_sub_dt <- summ_sub_dt %>% select(-label2) %>% ungroup() %>%
        mutate(across(-all_of(c("sequence", "aligned_sequence", "embedding","lev_dist","accuracy","identity","qcovs","label","split_metadata")), \(x) paste(cur_column(), replace_na(x, 0),sep=":"))) %>% 
        unite(col=metadata, -c(sequence, aligned_sequence, embedding,lev_dist,accuracy,identity,qcovs,label,split_metadata), sep="/") %>%
