...
option_list <- list(
  ...
  make_option(c("--num_targets"), "Number of targets to keep per anchor", type="integer",
              default=1)
)
...
if (!file.exists(all_satc_filtered)) {
  system(paste(file.path(opt$satc_util_bin, "satc_filter"), 
               ...
               "-d", opt$anchor_file, "-n", opt$num_targets))
}
...
# read in the dumped satc file (contains opt$num_targets targets per anchor)
# we want to now rank the targets by count for each sample-anchor pair so that when we
# pivot the data we can grab the top target per anchor in each cluster
...
if (!is_empty(missing_clusters)) {
  missing_df <- map_dfr(missing_clusters, \(x) data.frame(
    sample=unique(head(wide_satc$sample))[1],
    anchor=representative_anchors[x],
    target=rep(strrep("N", nchar(representative_anchors[x])), opt$num_targets),
    count=0,
    cluster_id=x,
    rank=1))
...
if (opt$num_targets == 1){
  ...
  for (i in 1:opt$num_targets) {
    ...
    dna <- Biostrings::DNAStringSet(seqs$seq)
    names(dna) <- seqs$sample
    writeXStringSet(dna, paste0(opt$output_prefix, "_sample_sequences_target_", i, ".fasta"))
