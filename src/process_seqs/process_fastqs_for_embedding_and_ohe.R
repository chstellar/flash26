...
option_list <- list(
  ...
  make_option(c("--sample_sheet"), "Path to original SPLASH sample_sheet.txt file", type="character"),
...
if (!file.exists(opt$anchor_file) | !file.exists(opt$cluster_file) | is.null(opt$sample_sheet) | is.null(opt$output_prefix)) {
...
satc_files <- fread(opt$sample_sheet, header=F, col.names=c("sample_id", "fastq_file"))
...
if (!file.exists(all_satc_file)) {
  ...
  system(paste("mv", file.path(temp_dir, "all_satc_merged_no_anchor.txt"), all_satc_file))
}

if (opt$single_cell) {
  # exit since this is not supported for single cell data
  stop("Single Cell data is not supported for this script. Snakemake will default to the original process_splash_data_for_embedding.R script.")
}
