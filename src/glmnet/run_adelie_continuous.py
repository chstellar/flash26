...
def get_metadata_columns(metadata, min_samples=50):
    ...
    filtered_metadata = filtered_metadata.loc[:, ~filtered_metadata.columns.isin(filtered_metadata_discrete.columns)]

    # the columns may not have been read in as numeric but we need to check if they are numeric
    # if they are not numeric, do not include them in the continuous columns
    filtered_metadata = filtered_metadata.apply(lambda x: pd.to_numeric(x, errors='ignore'))
    
    # now check if the columns are continuos by only including columns that are numeric
    filtered_metadata_continuous = filtered_metadata.loc[:, filtered_metadata.apply(lambda x: pd.api.types.is_numeric_dtype(x), axis=0)]
...
def merge_and_split_data_continuous(data, metadata, metadata_col, train_prop=0.7):
    metadata = metadata[["sample_name", metadata_col]]
    merged_data = pd.merge(data, metadata, on='sample_name', how='left')
    merged_data = merged_data.dropna(subset=[metadata_col])
...
def train_adelie_model_multinomial(X_train, y_train,n_threads=1):
...
def train_adelie_model_gaussian(X_train, y_train, n_threads=1):
    model = ad.GroupElasticNet(solver="cv_grpnet", family="gaussian")
    model.fit(X_train.astype(np.float64), y_train.astype(np.float64), n_threads=n_threads)
...
def main():
    ...
    discrete_metadata_columns, continuous_metadata_columns = get_metadata_columns(metadata, min_samples=args.min_samples)
    
    all_model_features = None

    # Iterate over the metadata columns
    with PdfPages(output_pdf) as pdf:
        for metadata_col in discrete_metadata_columns:
            ...
            try:
                model, oh = train_adelie_model_multinomial(X_train, y_train, n_threads=args.n_threads)
        ...
        for metadata_col in continuous_metadata_columns:
            print(f"Processing metadata column: {metadata_col}")
            print()
            
            X_train, X_test, y_train, y_test, model_cols = merge_and_split_data_continuous(data, metadata, metadata_col)
            ...
            # plot the distro of train vs test data
            plt.figure()
            plt.hist(y_train, bins=50, alpha=0.5, label='train')
            plt.hist(y_test, bins=50, alpha=0.5, label='test')
            plt.legend()
            plt.title(f"{metadata_col} train vs test data")
            pdf.savefig()
            plt.close()

            try:
                model = train_adelie_model_gaussian(X_train, y_train, n_threads=args.n_threads)
            except Exception as e:
                print(f"Failed to train model for {metadata_col}: {e}")
                continue
            ...
            print(f"R2 score for {metadata_col}: {r2:.2f}")

            # plot the predicted vs actual values
            # add a line X=Y to show perfect predictions
            plt.figure()
            plt.scatter(y_test, yhat)
            plt.title(f"{metadata_col} predicted vs actual values. R2: {r2:.2f}")
            plt.xlabel("Actual")
            ...
            # get the feature names for the nonzero coefficients
            # get the feature names for the nonzero coefficients
            metadata_categories = ["beta"]
            model_features = [f"{feature}+{category}" for feature in model_cols for category in metadata_categories]

            # get the names and values of the nonzero coefficients
            model_features = pd.DataFrame(model_features, columns=["feature"])
            model_features["coefficients"] = coef.toarray().flatten()
            model_features = model_features[model_features["coefficients"] != 0]
            # separate the feature names into the feature and category
            model_features["feature"] = model_features["feature"].str.split("+")
            model_features["classes"] = model_features["feature"].str[1]
            model_features["feature"] = model_features["feature"].str[0]

            model_features["metadata_category"] = metadata_col
            model_features["accuracy"] = r2
