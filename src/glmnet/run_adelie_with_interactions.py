...
def create_interaction_matrix(df, columns):
    """Creates a matrix of interaction terms for specified columns in a dataframe."""

    interaction_df = pd.DataFrame()

    for i, col1 in enumerate(columns):
        for col2 in columns[i+1:]:
            interaction_df[col1 + ':' + col2] = df[col1] * df[col2]

    return interaction_df

def train_interaction_model(X_train, y_train,n_threads=1):
    oh = OneHotEncoder(sparse_output=False, handle_unknown="ignore")
    y_train2 = oh.fit_transform(y_train[:, np.newaxis])
    
    model = ad.GroupElasticNet(solver="cv_grpnet", family="multinomial")
    model.fit(X_train, y_train2, n_threads=n_threads)
...
def main():
    ...
    output_interactions_coef = output_prefix + "_nonzero_interactions_coefficients.tsv"
    ...
    all_interaction_model_features = None
    ...
    with PdfPages(output_pdf) as pdf:
        for metadata_col in metadata_columns:
            ...
            # create a reduced matrix of features using only the nonzero ones
            print(f"Reducing matrix to nonzero features for {metadata_col}")
            cols_to_keep = feature_names.isin(unique_features)
            X_train_temp = X_train[:, cols_to_keep]
            X_test_temp = X_test[:, cols_to_keep]

            # create a new matrix with the interaction terms 
            # this throws some performance errors fix if possible
            print(f"Creating interaction terms matrix for {metadata_col}")
            X_train_interactions = create_interaction_matrix(pd.DataFrame(X_train_temp, columns=feature_names[cols_to_keep]), feature_names[cols_to_keep])
            X_test_interactions = create_interaction_matrix(pd.DataFrame(X_test_temp, columns=feature_names[cols_to_keep]), feature_names[cols_to_keep])

            # concatenate the interaction matrix with the original matrix
            X_train_combined = np.concatenate([X_train, X_train_interactions], axis=1)
            X_test_combined = np.concatenate([X_test, X_test_interactions], axis=1)

            # convert the X matrices to fortran arrays and save the column names
            interaction_names = list(feature_names)
            interaction_names.extend(X_train_interactions.columns)
            X_train_combined = np.asfortranarray(X_train_combined)
            X_test_combined = np.asfortranarray(X_test_combined)

            # train the model with the interaction terms
            print(f"Training model with interaction terms for {metadata_col}")
            interaction_model, interaction_oh = train_interaction_model(X_train_combined, y_train, n_threads=args.n_threads)

            # get the predictions and transform them back to the original categories
            yhat = interaction_model.predict(X_test_combined.astype(np.float64))
            if len(np.unique(yhat)) < 2:
                print(f"Skipping {metadata_col} as there are not enough unique predictions...")
                continue
            yhat_2d = np.zeros((yhat.size, yhat.max() + 1))
            ...
            try:
                y_pred = interaction_oh.inverse_transform(yhat).flatten()
                cm = confusion_matrix(y_test, y_pred)
                print(f"Confusion matrix for {metadata_col} with interactions")
                print(cm)
            except Exception as e:
                print(f"Failed to transform predictions for {metadata_col}: {e}")
                continue
            ...
            for i in range(cm.shape[0]):
                for j in range(cm.shape[1]):
                    plt.text(j, i, f"{cm[i, j]}", ha='center', va='center')
            if cm.shape[0] > 2:
                plt.title(f"{metadata_col} (w/ interactions)\nAccuracy: {accuracy:.2f}")
            else:
                plt.title(f"{metadata_col} (w/ interactions)\nAccuracy: {accuracy:.2f}, Specificity: {specificity:.2f}, Sensitivity: {sensitivity:.2f}")
            plt.xlabel("Predicted")
            plt.ylabel("True")
            ...
            # get the feature names for the nonzero coefficients
            interaction_metadata_categories = interaction_oh.categories_[0]
            interaction_model_features = [f"{feature}+{category}" for feature in interaction_names for category in interaction_metadata_categories]

            interaction_model_features = pd.DataFrame(interaction_model_features, columns=["feature"])
            interaction_model_features["coefficient"] = interaction_coef.toarray().flatten()
            interaction_model_features = interaction_model_features[interaction_model_features["coefficient"] != 0]
            # separate the feature names into the feature and category
            interaction_model_features["feature"] = interaction_model_features["feature"].str.split("+")
            interaction_model_features["category"] = interaction_model_features["feature"].str[1]

            interaction_model_features["feature"] = interaction_model_features["feature"].str[0]
            interaction_model_features = interaction_model_features.groupby("feature").agg({"coefficient": list, "category": list}).reset_index()
            interaction_model_features["classes"] = interaction_model_features["category"].apply(lambda x: "[" + ",".join(map(str, x)) + "]")
            interaction_model_features["coefficients"] = interaction_model_features["coefficient"].apply(lambda x: "[" + ",".join(map(str, x)) + "]")
            interaction_model_features["metadata_category"] = metadata_col
            ...
            if specificity is None:
                ...
                interaction_model_features["sensitivity"] = sensitivity

            interaction_model_features = interaction_model_features[["metadata_category", "feature", "accuracy", "sensitivity", "specificity", "classes", "coefficients"]]
            if all_interaction_model_features is None:
                all_interaction_model_features = interaction_model_features
            else:
                all_interaction_model_features = pd.concat([all_interaction_model_features, interaction_model_features], axis=0)
        ...
        if all_model_features is None:
            ...
            all_interaction_model_features.to_csv(output_interactions_coef, sep="\t", index=False, float_format="%.4f")
