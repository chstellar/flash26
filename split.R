...
option_list <- list(
  ...
  make_option(c("--metadata_categories"), type = "character", default = "",
              help = "Comma-separated metadata_category values to plot. Empty plots all categories.", metavar = "character"),
  make_option(c("--split_metadata_col"), type = "character", default = "",
              help = "Optional metadata column used to split detailed plots within one output PDF.", metavar = "character"),
...
metadata_categories_to_plot <- str_split_1(opt$metadata_categories, ",") %>% str_trim()
metadata_categories_to_plot <- metadata_categories_to_plot[nchar(metadata_categories_to_plot) > 0]
if (length(metadata_categories_to_plot) > 0) {
  dt <- dt %>% filter(metadata_category %in% metadata_categories_to_plot)
  dt2 <- dt2 %>% filter(metadata_category %in% metadata_categories_to_plot)
  if (nrow(dt) == 0 && nrow(dt2) == 0) {
    stop(paste0("No rows matched --metadata_categories: ", opt$metadata_categories), call. = FALSE)
  }
}
if (!is.null(opt$split_metadata_col) && nchar(opt$split_metadata_col) > 0 &&
    !opt$split_metadata_col %in% colnames(all_metadata)) {
  stop(paste0("Split metadata column '", opt$split_metadata_col, "' was not found in ", opt$metadata),
       call. = FALSE)
}
...
pdf(opt$output, width=16, height=10)
...
for (category in categories) {
  tryCatch({
    ...
    metadata_cols_to_keep <- unique(c("sample_name", metadata_source_col, opt$split_metadata_col))
    metadata_cols_to_keep <- metadata_cols_to_keep[nchar(metadata_cols_to_keep) > 0]
    my_metadata <- as_tibble(all_metadata) %>%
      select(all_of(metadata_cols_to_keep)) %>%
      dplyr::rename(metadata:=!!metadata_source_col)
    if (!is.null(opt$split_metadata_col) && nchar(opt$split_metadata_col) > 0) {
      my_metadata <- my_metadata %>%
        dplyr::rename(split_metadata:=!!sym(opt$split_metadata_col)) %>%
        filter(!is.na(split_metadata), nchar(as.character(split_metadata)) > 0)
    } else {
      my_metadata <- my_metadata %>% mutate(split_metadata = "all")
    }
    ...
    for (j in 1:nrow(interesting_clusters)) {
      ...
      if (is_quantitative_target) {
        summ_sub_dt <- seq_sub %>% left_join(my_metadata,by="sample_name") %>%
          filter(!is.na(metadata), !is.na(split_metadata)) %>%
          group_by(sequence,embedding,lev_dist,aligned_sequence,split_metadata) %>%
        ...
        summ_sub_dt <- seq_sub %>% left_join(my_metadata,by="sample_name") %>%
          filter(!is.na(split_metadata)) %>%
          group_by(sequence,embedding,lev_dist,aligned_sequence,split_metadata,metadata) %>%
      ...
      for (split_value in sort(unique(as.character(p_sub$split_metadata)))) {
        p_split <- p_sub %>% filter(as.character(split_metadata) == split_value)
        if (nrow(p_split) == 0) {
          next
        }
        split_subtitle <- ifelse(nchar(opt$split_metadata_col) > 0,
                                 paste(opt$split_metadata_col, split_value, sep=": "),
                                 "")

        if (is_quantitative_target) {
          p_split <- p_split %>% mutate(color_value = mean_metadata)
          p2 <- p_split %>%
            ...
            ggtitle(paste(category, my_cluster, sep=" | "),
                    subtitle=paste(c(split_subtitle,
                                     paste("Color: mean observed", metadata_source_col)),
                                   collapse=" | ")) +
            theme(panel.grid.minor.y = element_blank())
          print(p2)
          summ_out_dt <- p_split %>% select(-any_of("label2")) %>%
          ...
          missing_class_cols <- setdiff(all_classes, colnames(p_split))
          for (missing_class_col in missing_class_cols) {
            p_split[[missing_class_col]] <- 0
          }
          p_split <- p_split %>%
            mutate(across(all_of(all_classes), \(x) replace_na(x, 0))) %>%
            mutate(total_samples = rowSums(across(all_of(all_classes))))
          ...
          for (class_to_plot in classes_to_plot) {
            if (!class_to_plot %in% colnames(p_split)) {
              next
            }
            p_class <- p_split %>%
            ...
            p2 <- p_class %>%
              ...
              ggtitle(paste(category, my_cluster, sep=" | "),
                      subtitle=paste(c(split_subtitle,
                                       paste("Color: proportion", class_to_plot)),
                                     collapse=" | ")) +
              theme(panel.grid.minor.y = element_blank())
            print(p2)
          }

          summ_out_dt <- p_split %>% select(-any_of("label2")) %>% ungroup() %>%
        ...
        summ_out_dt <- summ_out_dt %>%
          mutate(split_metadata_column = opt$split_metadata_col,
                 split_metadata_value = split_value) %>%
...
all_features_summary %>%
  relocate(metadata_category, feature, cluster) %>%
  write_tsv(file = default_summary_path)
message(paste0("Wrote split plot summary to: ", default_summary_path))
unannotated_summary %>% write_tsv(file = str_replace(opt$output, ".pdf", "_unannotated.tsv"))
all_blastp_summary %>%
  relocate(metadata_category, feature, cluster) %>%
  write_tsv(file = str_replace(opt$output, ".pdf", "_blastp_all.tsv"))
all_blast_summary %>%
  relocate(metadata_category, feature, cluster) %>%
  write_tsv(file = str_replace(opt$output, ".pdf", "_blast_all.tsv"))
