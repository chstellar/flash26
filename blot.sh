...
# wolbachia
DATASET="${DATASET:-260826-01-2flies-wolbachia}"
RESULTS_DIR="${RESULTS_DIR:-${PROJECT_DIR}/results/${DATASET}/filter1/shiftDist-levFilter/hyena/normalized}"
METADATA="${METADATA:-/scratch/users/jiamuyu/proj_botryllus/splash2/260826_00_2flies-wolbachia/metadata.csv}"
...
COMPACTOR_SUMMARY="${COMPACTOR_SUMMARY:-${RESULTS_DIR}/260826-01-2flies-wolbachia_hyena_adelie_results_top20000_target1_k54_s54_trainProp0.8_nonzero_coefficients_blast_annotated_plots_summary_compactor.tsv}"
GENE_LABEL_SIZE="${GENE_LABEL_SIZE:-8}"
HIST_LABEL_SCALE="${HIST_LABEL_SCALE:-1.25}"
AXIS_TITLE_SIZE="${AXIS_TITLE_SIZE:-16}"
AXIS_TEXT_SIZE="${AXIS_TEXT_SIZE:-14}"
PLOT_TITLE_SIZE="${PLOT_TITLE_SIZE:-16}"
PLOT_SUBTITLE_SIZE="${PLOT_SUBTITLE_SIZE:-14}"
LABEL_LINEHEIGHT="${LABEL_LINEHEIGHT:-0.92}"
COUNT_LABEL_VJUST="${COUNT_LABEL_VJUST:-0.5}"
CLASS_ORDER="${CLASS_ORDER:-uninfected,infected}"
CLASS_COLORS="${CLASS_COLORS:-uninfected:#7F7F7F,infected:#D55E00}"
...
# GENE_LABEL_SIZE="${GENE_LABEL_SIZE:-6}"
# HIST_LABEL_SCALE="${HIST_LABEL_SCALE:-1.25}"
# AXIS_TITLE_SIZE="${AXIS_TITLE_SIZE:-16}"
