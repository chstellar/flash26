...
configfile: f"{Path(workflow.snakefile).stem}.yaml"
dataset_table_path = Path(glob.glob(f"{Path(workflow.configfiles[0]).stem}*.csv")[0])
...
rule process_genome_to_sample_sequences:
    ...
    params:
        ...
        satc_util_bin = Path(config["satc_util_bin"]), # satc utils are typically located in the same directory as the main splash binary.
        anchor_len = config["options"]["anchor_length"],
        target_len = config["options"]["target_length"]
    output:
    ...
    shell:"""
        ...
        --anchor_len {params.anchor_len} --target_len {params.target_len}
    """
